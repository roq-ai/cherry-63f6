...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getOnboardingFlows() {
    const data = await prisma.onboarding_flow
      .withAuthorization({
        roqUserId,
        tenantId: user.tenantId,
        roles: user.roles,
      })
      .findMany(convertQueryToPrismaUtil(req.query, 'onboarding_flow'));
  ...
  async function createOnboardingFlow() {
    ...
    const data = await prisma.onboarding_flow.create({
      data: body,
    });
    return res.status(200).json(data);
  }
