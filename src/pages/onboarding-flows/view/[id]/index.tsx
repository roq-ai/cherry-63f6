...
import { getOnboardingFlowById } from 'apiSdk/onboarding-flows';
...
function OnboardingFlowViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<OnboardingFlowInterface>(
    ...
    () =>
      getOnboardingFlowById(id, {
        relations: ['template', 'platform', 'feedback'],
      }),
  );
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          <Text as="h1" fontSize="2xl" fontWeight="bold">
            Onboarding Flow Detail View
          </Text>
          {hasAccess('onboarding_flow', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/onboarding-flows/edit/${data?.id}`} passHref legacyBehavior>
              <Button
              ...
              {hasAccess('feedback', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/feedbacks/create?onboarding_flow_id=${data?.id}`}>
